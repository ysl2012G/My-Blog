...
//        }
        ...
        if (!uri.startsWith(contextPath + "/admin/js")
                && !uri.startsWith(contextPath + "/admin/image")
        ...
        //拦截/admin/**,注入loggin_user
//        String contextPath = httpServletRequest.getContextPath();
//        String uri = httpServletRequest.getRequestURI();
        Subject subject = SecurityUtils.getSubject();
//        if (!uri.startsWith(contextPath + "/admin/js") && !uri.startsWith(contextPath + "/admin/image") && !uri.startsWith(contextPath + "/admin/plugins") &&
//                uri.startsWith(contextPath + "/admin") && !uri.startsWith(contextPath + "/admin/login") && (subject.isAuthenticated() || subject.isRemembered())) {
        if (subject.isAuthenticated() || subject.isRemembered()) {
            UserVo userVo = (UserVo) subject.getPrincipal();
            request.setAttribute("login_user", userVo);
            LOGGER.debug("request has set attribute: login_user");
        }
